...
const actions = {
  ...
  authLogin(context, payload) {
    ...
    workerMgr.hashPassword(payload.username, payload.password).then(hPass => {
      authSvc
        .login({ id: payload.username, password: hPass, want_csrf: true })
        .then(response => {
          context
            .dispatch('sessionStoreServerSession', {
  ...
  authConfirmEmail(context, payload) {
    context.commit(mt.AUTH_START_WORK)
    authSvc
      .confirmEmail({ token: payload.token })
      .then(response => {
        context.commit(mt.AUTH_STOP_WORK)
  ...
  authResendConfirmEmail(context, payload) {
    context.commit(mt.AUTH_START_WORK)
    authSvc
      .resendConfirmEmail({ email: payload.email })
      .then(response => {
        context.commit(mt.AUTH_STOP_WORK)
