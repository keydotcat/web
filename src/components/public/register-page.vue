    ...
    <form class="form-login" @submit="submit">
      ...
      <template v-for="(val, field) in static_errors">
        <div class="form-group">
          ...
          <input
            :type="field.indexOf('password') == 0 ? 'password' : ''"
            v-model="form[field]"
            :disabled="working"
            @change="setDirty(field)"
            class="form-control"
            :class="{ 'is-invalid': errs[field].length > 0 }"
          />
...
export default {
  ...
  computed: {
    ...
    emailError() {
      switch (this.$store.state.public.errorFields.email) {
        case 'invalid':
    ...
    usernameError() {
      var authState = this.$store.state.public
    ...
    fullnameError() {
      if (this.$store.state.public.errorFields.fullname) {
